-- 409A Valuation App - Final Consolidated Schema
-- This single file sets up everything correctly
...
-- Remove and recreate role constraint to handle all possible values
ALTER TABLE public.organization_members DROP CONSTRAINT IF EXISTS organization_members_role_check;
ALTER TABLE public.organization_members ADD CONSTRAINT organization_members_role_check
    CHECK (role IN ('super_admin', 'org_owner', 'org_admin', 'appraiser', 'viewer', 'member', 'admin', 'owner'));
...
-- Companies RLS Policies (drop all first to avoid conflicts)
DO $$
BEGIN
...
-- Super admins full access (separate policy to avoid conflicts)
...
-- Permissive policies for other tables (for development)
DO $$
DECLARE
    t text;
BEGIN
    -- List of tables to apply permissive policies
    FOR t IN SELECT unnest(ARRAY[
        ...
        'reports',
        'report_templates',
        'report_variable_mappings',
        'invitations',
        'activity_logs'
    ])
    LOOP
        -- Drop existing "All operations" policy if it exists
        EXECUTE format('DROP POLICY IF EXISTS "All operations on %I" ON public.%I', t, t);
        -- Create new permissive policy
        EXECUTE format('CREATE POLICY "All operations on %I" ON public.%I FOR ALL USING (true) WITH CHECK (true)', t, t);
    END LOOP;
END $$;
...
BEGIN
    ...
    LOOP
        ...
        EXECUTE format('CREATE TRIGGER update_%I_updated_at BEFORE UPDATE ON public.%I FOR EACH ROW EXECUTE FUNCTION update_updated_at_column()', t.table_name, t.table_name);
    END LOOP;
END $$;
...
BEGIN
    ...
    IF v_user_id IS NOT NULL THEN
        ...
        -- Create/update user profile with super admin status (without email initially)
        INSERT INTO user_profiles (id, is_super_admin)
        VALUES (v_user_id, true)
        ON CONFLICT (id) DO UPDATE SET is_super_admin = true;

        -- Update email separately to avoid column errors
    ...
    ELSE
        RAISE NOTICE 'User not found: eran@bridgeland-advisors.com - Please ensure you have logged in at least once';
    END IF;
END $$;
...
SELECT
    ...
    (SELECT is_super_admin FROM user_profiles WHERE id = (SELECT id FROM auth.users WHERE email = 'eran@bridgeland-advisors.com' LIMIT 1)) as your_super_admin_status;