...
interface UseTemplateEngineReturn {
  generateReport: (template: ReportTemplate, data: Record<string, any>, options?: { status?: 'draft' | 'final'; watermark?: boolean }) => Promise<GeneratedReport>;
  validateData: (template: ReportTemplate, data: Record<string, any>) => { isValid: boolean; errors: string[]; warnings: string[] };
