...
export function useReportTemplates({
  ...
  const saveTemplate = useCallback(async (template: Omit<ReportTemplate, 'id' | 'createdAt' | 'updatedAt' | 'version'>) => {
    try {
      setError(null);
  ...
  const duplicateTemplate = useCallback(async (templateId: string, newName: string) => {
    try {
      setError(null);
      const duplicatedTemplate = await ReportTemplateService.duplicateTemplate(templateId, newName);
  ...
  const saveVariableMapping = useCallback(async (mapping: Omit<VariableMapping, 'id'>) => {
    try {
      setError(null);
