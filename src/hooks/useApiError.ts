...
export interface ApiError {
  message: string;
  type?: 'validation' | 'authentication' | 'authorization' | 'not_found' | 'rate_limit' | 'server' | 'network';
  status?: number;
...
export function useOptimisticUpdate<T>(initialData: T) {
  ...
  const performOptimisticUpdate = useCallback((newData: T, updateFn: () => Promise<T>) => {
