...
describe('Companies API Integration Tests', () => {
  ...
  describe('Complete CRUD Workflow', () => {
    ...
    it('should complete full company CRUD lifecycle', async () => {
      ...
      const getRequest = new NextRequest(
        `http://localhost:3000/api/companies/${createdCompanyId}`,
        {
          method: 'GET',
        }
      )

      const getResponse = await getSingle(getRequest, {
        params: { id: createdCompanyId.toString() },
      })
      ...
      const updateResponse = await updateSingle(updateRequest, {
        params: { id: createdCompanyId.toString() },
      })
      ...
      const deleteResponse = await deleteSingle(deleteRequest, {
        params: { id: createdCompanyId.toString() },
      })
      expect(deleteResponse.status).toBe(204)

      // Step 6: Verify deletion by attempting to retrieve
      const verifyDeleteRequest = new NextRequest(
        `http://localhost:3000/api/companies/${createdCompanyId}`,
        {
          method: 'GET',
        }
      ...
      const verifyDeleteResponse = await getSingle(verifyDeleteRequest, {
        params: { id: createdCompanyId.toString() },
      })
  ...
  describe('Validation Error Flows', () => {
    it('should handle validation errors throughout the workflow', async () => {
      ...
      const invalidUpdateResponse = await updateSingle(invalidUpdateRequest, {
        params: { id: '1' },
      })
  ...
  describe('Pagination and Filtering', () => {
    it('should handle complex pagination and filtering scenarios', async () => {
      ...
      const filterRequest = new NextRequest(
        'http://localhost:3000/api/companies?industry=Technology&employees_min=10',
        {
          method: 'GET',
        }
      )
