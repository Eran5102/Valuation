...
describe('Valuations API Integration Tests', () => {
  ...
  describe('Valuation Creation Workflow', () => {
    it('should create valuation with complete validation flow', async () => {
      ...
      mockValuationRoutes.POST.mockResolvedValue(new Response(JSON.stringify({
        success: true,
        data: {
          id: 1,
          companyId: 1,
      ...
      expect(result.data.fairMarketValue).toBe(2.50);
      ...
      for (const transition of statusTransitions) {
        mockValuationRoutes.PUT.mockResolvedValue(
          new Response(JSON.stringify({
            ...
            data: transition.expectedStatus === 200 ? { id: 1, status: transition.to } : undefined,
            error: transition.expectedStatus === 400 ? {
              type: 'business_logic',
  ...
  describe('Performance and Scalability', () => {
    it('should handle large datasets efficiently', async () => {
      ...
      mockValuationRoutes.GET.mockResolvedValue(new Response(JSON.stringify({
        success: true,
      ...
      const largeDatasetRequest = new NextRequest('http://localhost:3000/api/valuations?limit=1000', {
        method: 'GET'
      });
  ...
  describe('Data Consistency Integration', () => {
    it('should maintain data consistency across related operations', async () => {
      ...
      expect(result.relatedUpdates.company.currentFMV).toBe(5.00);
    });
  });
});