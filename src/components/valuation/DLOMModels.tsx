...
const DLOMModels: React.FC<DLOMModelsProps> = ({ assumptions, valuationId }) => {
  ...
  const calculateGhaidarov = (): number => {
    ...
    const sigmaSquaredT =
      Math.log(2 * (expVarianceT - varianceTimesT - 1)) - 2 * Math.log(varianceTimesT)
  ...
  const calculateLongstaff = (): number => {
    ...
    const putValue =
      S *
      (NSqrtSigmaSquaredTOver2 * expNegSigmaSquaredTOver8 -
  ...
  const columns: ColumnDef<ModelData>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="text-right">
            <div className="font-medium text-foreground">{formatPercentage(contribution)}</div>
            <div className="text-sm text-muted-foreground">to weighted avg</div>
          </div>
        )
      },
    },
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3 lg:grid-cols-6">
            ...
            <div>
              ...
              <Input
                ...
                readOnly
              />
              <p className="mt-1 text-xs text-muted-foreground">From assumptions</p>
            </div>
            <div>
              <label className="mb-1 block text-sm font-medium text-foreground">
                Risk-Free Rate (%)
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-foreground">
              {formatPercentage(
                (results.chaffee + results.finnerty + results.ghaidarov + results.longstaff) / 4
              )}
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-lg font-bold text-foreground">
              {formatPercentage(
                Math.min(results.chaffee, results.finnerty, results.ghaidarov, results.longstaff)
              )}{' '}
              -{' '}
              {formatPercentage(
                Math.max(results.chaffee, results.finnerty, results.ghaidarov, results.longstaff)
              )}
            </div>
          </CardContent>
        </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            ...
            {(weights.chaffee + weights.finnerty + weights.ghaidarov + weights.longstaff).toFixed(
              0
            )}
            %
          </CardDescription>
