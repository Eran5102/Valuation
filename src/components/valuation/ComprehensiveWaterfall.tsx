...
export default function ComprehensiveWaterfall({ companyId, capTableConfig }: ComprehensiveWaterfallProps) {
  ...
  const totalDistributed = mockWaterfallResults.reduce((sum, result) => sum + result.totalDistribution, 0);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Distribution waterfall analysis and visualizations</p>
        </div>
                ...
                <TableBody>
                  {mockWaterfallResults.map((result) => (
                    <TableRow key={result.security}>
                      ...
                      <TableCell className="text-right">{formatCurrency(result.liquidationPreference)}</TableCell>
                      <TableCell className="text-right">{formatCurrency(result.participationAmount)}</TableCell>
                      <TableCell className="text-right font-medium">{formatCurrency(result.totalDistribution)}</TableCell>
                      <TableCell className="text-right">${result.valuePerShare.toFixed(2)}</TableCell>
                  ...
                  <TableRow className="border-t-2 border-primary/20 font-bold">
                    ...
                    <TableCell className="text-right">{formatNumber(mockWaterfallResults.reduce((sum, r) => sum + r.shares, 0))}</TableCell>
                    <TableCell className="text-right">{formatCurrency(mockWaterfallResults.reduce((sum, r) => sum + r.liquidationPreference, 0))}</TableCell>
                    <TableCell className="text-right">{formatCurrency(mockWaterfallResults.reduce((sum, r) => sum + r.participationAmount, 0))}</TableCell>
                    <TableCell className="text-right font-bold text-primary">{formatCurrency(totalDistributed)}</TableCell>
                    <TableCell></TableCell>
                    <TableCell></TableCell>
                  </TableRow>
                </TableBody>
            ...
            <CardContent>
              <div className="h-64 bg-muted/30 rounded-lg flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-muted-foreground">Interactive waterfall chart will be displayed here</p>
                  <p className="text-sm text-muted-foreground mt-1">Showing distribution flow from liquidation preferences to pro rata participation</p>
                </div>
              </div>
            </CardContent>
          </Card>
      ...
      {!isCalculated && (
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-foreground mb-2">Ready to Calculate Waterfall</h3>
