...
export function OptionsRow({
  ...
  const handleFieldChange = useCallback(
    (field: keyof OptionsWarrants, value: unknown) => {
      if (isEditing) {
      ...
      <tr className="bg-blue-50">
        ...
        <td className="whitespace-nowrap px-6 py-4">
          <span className="text-gray-900">
            {formatCurrency(
              ((getValue('numOptions') as number) || 0) *
                ((getValue('exercisePrice') as number) || 0)
            )}
          </span>
        </td>
        <td className="whitespace-nowrap px-6 py-4">
  ...
  return (
    <tr className="hover:bg-gray-50">
      <td className="whitespace-nowrap px-6 py-4 text-sm">
        <span
          className={`inline-flex rounded-full px-2 py-1 text-xs font-semibold ${getTypeColor(option.type)}`}
        >
          {option.type}
        </span>
      </td>
