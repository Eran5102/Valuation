...
import { Save, Eye, Settings, Plus, Copy, FolderPlus, Upload, MoreHorizontal, Edit2, Trash2, GripVertical } from 'lucide-react';
...
export function TemplateEditor({ template, onSave, onPreview, className }: TemplateEditorProps) {
  ...
  const handleBlockUpdate = useCallback((sectionId: string, blockId: string, updates: Partial<TemplateBlock>) => {
    const updatedSections = currentTemplate.sections.map(section => {
  ...
  const handleBlockAdd = useCallback((sectionId: string, newBlock: TemplateBlock, index?: number) => {
    const updatedSections = currentTemplate.sections.map(section => {
      if (section.id !== sectionId) return section;
  ...
  const handleSectionRename = useCallback((sectionId: string) => {
  ...
  const handleSectionReorder = useCallback((dragIndex: number, hoverIndex: number) => {
    ...
    <div className={`flex flex-col h-full bg-background ${className}`}>
      ...
      <div className="flex-1 overflow-hidden">
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as any)} className="h-full">
          <TabsList className="border-b border-border rounded-none">
          ...
          <TabsContent value="editor" className="h-full p-0 m-0">
            <DndContext onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
                        ...
                        >
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="start" side="bottom">
                              ...
                              <DropdownMenuItem
                                onClick={() => handleSectionReorder(index, Math.min(currentTemplate.sections.length - 1, index + 1))}
                    ...
                    {currentSection && (
                      <TemplateCanvas
                        ...
                        onBlockDelete={(blockId) =>
                          handleBlockDelete(selectedSection, blockId)
                        }
              ...
              <DragOverlay>
                {draggedBlock && (
                  <div className="bg-card border border-border rounded shadow-lg p-2 opacity-80">
                    {draggedBlock.type}
                  </div>
                )}
              </DragOverlay>
      ...
      <Dialog open={showRenameDialog} onOpenChange={setShowRenameDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Enter a new title for the section.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
