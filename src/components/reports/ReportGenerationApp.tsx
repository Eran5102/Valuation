...
const PageHeader = dynamic(() => import('@/components/ui/page-header').then(mod => ({ default: mod.PageHeader })), {
  loading: () => <div className="h-16 animate-pulse bg-muted rounded" />
});

const SimpleTable = dynamic(() => import('@/components/ui/simple-table').then(mod => ({ default: mod.SimpleTable })), {
  loading: () => <div className="h-64 animate-pulse bg-muted rounded" />
});

const SelectableCardGrid = dynamic(() => import('@/components/ui/selectable-card-grid').then(mod => ({ default: mod.SelectableCardGrid })), {
  loading: () => <div className="h-48 animate-pulse bg-muted rounded" />
});

const StepIndicator = dynamic(() => import('@/components/ui/step-indicator').then(mod => ({ default: mod.StepIndicator })), {
...
export function ReportGenerationApp({ preselectedValuationId }: ReportGenerationAppProps) {
  const [step, setStep] = useState<'selectValuation' | 'selectTemplate' | 'preview' | 'generate'>('selectValuation');
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      const report = TemplateEngine.processTemplate(selectedTemplate || standard409ATemplate, valuationData, {
        status: 'final',
        watermark: false
      });
  ...
  const valuationColumns = [
    ...
    },
    {
      ...
      header: 'Status',
      render: (valuation: Valuation) => (
        <Badge className={valuation.status === 'Final' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}>
          {valuation.status}
        </Badge>
      ),
    },
  ...
  const getPageTitle = () => {
    switch (step) {
      case 'selectValuation': return 'Report Generator';
      case 'selectTemplate': return 'Choose Template';
      case 'preview': return 'Generate Report';
      default: return 'Report Generator';
    }
  };

  const getPageDescription = () => {
    switch (step) {
      case 'selectValuation': return 'Select a valuation to generate a professional 409A report';
      case 'selectTemplate': return `Select the best template for ${selectedValuation?.company_name}`;
      case 'preview': return 'Review your selections and generate the final report';
      default: return 'Generate professional 409A valuation reports for your clients';
    }
  };
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-muted-foreground">
          Ready to generate your 409A report for <strong>{selectedValuation?.company_name}</strong> using the <strong>{selectedTemplate?.name}</strong> template
        </p>
      </div>
