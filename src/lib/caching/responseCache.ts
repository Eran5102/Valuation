...
class ResponseCache {
   ...
   */
  generateKey(request: NextRequest, options: CacheOptions = {}): string {
    const url = new URL(request.url)
    const { keyPrefix = 'api', vary = [] } = options

    // Base key components
    const keyParts = [keyPrefix, request.method, url.pathname, url.search]
  ...
  get(key: string): NextResponse | null {
    ...
    response.headers.set(
      'X-Cache-Age',
      Math.floor((Date.now() - entry.timestamp) / 1000).toString()
    )
   ...
   */
  async set(key: string, response: NextResponse, options: CacheOptions = {}): Promise<void> {
    const {
