...
class BlackScholesCalculator {
  static calculateOptionValue(
    ...
    const callValue = stockPrice * Math.exp(-dividendYield * timeToExpiration) * this.normalCDF(d1) -
      strikePrice * Math.exp(-riskFreeRate * timeToExpiration) * this.normalCDF(d2);

    const putValue = strikePrice * Math.exp(-riskFreeRate * timeToExpiration) * this.normalCDF(-d2) -
...
async function processValuationCalculation(job: any): Promise<any> {
  ...
  const shareClassValues = await calculateShareClassValues(shareClasses, enterpriseValue, assumptions);
  job.progress = 60;
...
async function processDataExport(job: any): Promise<any> {
  ...
  console.log(`Data export completed: ${filename} (${data.length} records, ${exportBuffer.length} bytes)`);
  return result;
}
...
async function calculateShareClassValues(shareClasses: any[], enterpriseValue: number, assumptions: any): Promise<any> {
  ...
  return {
    common: {
      ...
      totalValue: basePrice * (shareClasses.find(sc => sc.type === 'common')?.sharesOutstanding || 1000000)
    },
...
async function calculateOptionValues(shareClasses: any[], enterpriseValue: number, assumptions: any): Promise<any> {
