...
const percentage = z.number().min(0, 'Must be between 0 and 100').max(100, 'Must be between 0 and 100');
...
export const PreferenceTypeEnum = z.enum(['non-participating', 'participating', 'participating-with-cap']);
...
export const DLOMInputsSchema = z.object({
  ...
  volatility: z.number().min(0.01, 'Volatility must be greater than 0').max(5, 'Volatility seems unreasonably high'),
  riskFreeRate: z.number().min(0, 'Risk-free rate must be non-negative').max(0.3, 'Risk-free rate seems unreasonably high'),
  timeToExpiration: z.number().min(0.01, 'Time to expiration must be positive').max(30, 'Time to expiration seems unreasonably long'),
...
export const BreakpointSchema = z.object({
  id: z.number().optional(),
  name: requiredString,
  type: z.enum(['Liquidation Preference', 'Pro Rata', 'Option Exercise', 'Cap Reached', 'Conversion']),
  ...
  participatingSecurities: z.array(z.object({
    name: requiredString,
...
export const WaterfallInputSchema = z.object({
  ...
  cap_table: z.object({
    preferred_series: z.array(z.object({
      name: requiredString,
...
export const customValidators = {
  ...
  isValidIndustry: (industry: string): boolean => {
    const validIndustries = [
      ...
      'Media & Entertainment', 'Transportation', 'Agriculture', 'Other'
    ];
...
export const refinedSchemas = {
  ShareClassWithBusinessRules: ShareClassSchema.refine((data) => {
  ...
  ValuationProjectWithDateValidation: ValuationProjectSchema.refine((data) => {
