...
export class ComprehensiveWaterfallEngine {
    ...
    static calculateWaterfall(input_data: any): WaterfallResult {
        ...
        try {
            ...
            calculation_audit_trail.push(`- Cap table loaded: ${waterfall_input.cap_table.preferred_series.length} preferred series, ${waterfall_input.cap_table.option_pools.length} option pools`);
            ...
            if (!net_proceeds_result.validation_status.is_valid) {
                throw new Error(`Net proceeds validation failed: ${net_proceeds_result.validation_status.errors.join(', ')}`);
            }
            
            calculation_audit_trail.push(`- Net proceeds: $${net_proceeds_result.net_proceeds.toFixed(2)}`);
            calculation_audit_trail.push(`- Validation: ${net_proceeds_result.validation_status.confidence_level}`);
            ...
            calculation_audit_trail.push(`- ${breakpoint_analysis.breakpoints.length} breakpoints identified`);
            calculation_audit_trail.push(`- Complexity score: ${breakpoint_analysis.calculation_metadata.complexity_score}`);
            ...
            if (has_circular_dependencies) {
                ...
                calculation_audit_trail.push(`- Convergence achieved: ${circular_resolution_result.convergence_achieved}`);
                calculation_audit_trail.push(`- Iterations: ${circular_resolution_result.iteration_count}`);
                calculation_audit_trail.push(`- Final price per share: $${circular_resolution_result.final_price_per_share.toFixed(6)}`);
            } else {
            ...
            try {
                ...
                calculation_audit_trail.push(`- ${rvps_result.conversion_decisions.length} conversion decisions analyzed`);
                calculation_audit_trail.push(`- ${rvps_result.sequential_conversion_order.length} securities converting to common`);
                calculation_audit_trail.push(`- Total waived liquidation preferences: $${Object.values(rvps_result.waived_liquidation_preferences).reduce((sum, val) => sum.add(val), new Decimal(0)).toFixed(2)}`);
            } catch (rvps_error) {
                calculation_audit_trail.push(`- RVPS calculation failed: ${rvps_error instanceof Error ? rvps_error.message : 'Unknown RVPS error'}`);
                throw new Error(`RVPS calculation failed: ${rvps_error instanceof Error ? rvps_error.message : 'Unknown RVPS error'}`);
            }
            ...
            const total_distributed = Object.values(final_distributions)
                .reduce((sum, amount) => sum.add(amount), new Decimal(0));
            
            calculation_audit_trail.push(`- Total distributed: $${total_distributed.toFixed(2)}`);
            calculation_audit_trail.push(`- Distribution accuracy: ${total_distributed.div(net_proceeds_result.net_proceeds).mul(100).toFixed(4)}%`);
            ...
            calculation_audit_trail.push(`ERROR: ${error instanceof Error ? error.message : 'Unknown error'}`);
    ...
    private static calculateFinalDistributions(
        ...
        const staying_preferred = rvps_result.conversion_decisions
            .filter(decision => decision.decision === "liquidation_preference");
        ...
        if (remaining_proceeds.gt(0)) {
            ...
            const converting_securities = rvps_result.conversion_decisions
                .filter(decision => decision.decision === "convert");
            
            converting_securities.forEach(decision => {
                const series = cap_table.preferred_series.find(s => s.series_name === decision.security_name);
                if (series) {
            ...
            cap_table.preferred_series
                ...
                .forEach(series => {
                    ...
                    if (series.participation_cap) {
                        ...
                        const current_total = (distributions[series.series_name] || new Decimal(0)).add(participation_amount);
                        
                        if (current_total.gt(max_total)) {
                            const capped_participation = max_total.sub(distributions[series.series_name] || new Decimal(0));
                            distributions[series.series_name] = (distributions[series.series_name] || new Decimal(0)).add(capped_participation);
                        } else {
                            distributions[series.series_name] = (distributions[series.series_name] || new Decimal(0)).add(participation_amount);
                        }
                    } else {
                        distributions[series.series_name] = (distributions[series.series_name] || new Decimal(0)).add(participation_amount);
                    }
            ...
            if (circular_resolution) {
                Object.entries(circular_resolution.exercised_options).forEach(([pool_name, exercised_shares]) => {
    ...
    private static detectCircularDependencies(cap_table: CapTable): boolean {
        ...
        const has_net_exercise_options = cap_table.option_pools.some(pool => 
            pool.exercise_method === "net" || pool.exercise_method === "both"
        );
        
        // Check for complex conversion scenarios that might create circularity
        const has_complex_conversions = cap_table.preferred_series.some(series => 
            series.is_participating && series.participation_cap
        );
    ...
    static formatWaterfallResult(result: WaterfallResult): string {
        ...
        lines.push(`Distribution Accuracy: ${result.total_distributed.div(result.net_proceeds).mul(100).toFixed(4)}%`);
        ...
        if (result.circular_resolution) {
            ...
            lines.push(`Convergence Achieved: ${result.circular_resolution.convergence_achieved ? "YES" : "NO"}`);
            lines.push(`Iterations: ${result.circular_resolution.iteration_count}`);
            lines.push(`Final Price per Share: $${result.circular_resolution.final_price_per_share.toFixed(6)}`);
            
            const exercised_options = Object.entries(result.circular_resolution.exercised_options)
                .filter(([,shares]) => shares.gt(0));
