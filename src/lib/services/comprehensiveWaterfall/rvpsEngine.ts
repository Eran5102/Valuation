  ...
  static calculateRVPS(
    ...
    cap_table.preferred_series.forEach((series) => {
      ...
      const total_shares = cap_table.common_stock_shares.add(
        cap_table.preferred_series.reduce(
          (sum, s) => sum.add(s.shares_outstanding.mul(s.conversion_ratio)),
          new Decimal(0)
        )
      )

      const per_share_value = total_shares.gt(0) ? net_proceeds.div(total_shares) : new Decimal(0)
      const conversion_value = series.shares_outstanding
        .mul(series.conversion_ratio)
        .mul(per_share_value)
      ...
      conversion_decisions.push({
        ...
        conversion_breakpoint: liquidation_value.gt(0)
          ? liquidation_value.div(series.shares_outstanding.mul(series.conversion_ratio))
          : new Decimal(0),
        liquidation_value,
        conversion_value,
      ...
      const rvps_value = series.shares_outstanding.gt(0)
        ? optimal_value.div(series.shares_outstanding)
        : new Decimal(0)
    ...
    const total_conversion_impact = Object.values(waived_liquidation_preferences).reduce(
      (sum, val) => sum.add(val),
      new Decimal(0)
    )
