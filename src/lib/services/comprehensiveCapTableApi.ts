...
export class ComprehensiveCapTableService {
  ...
  static convertLegacyCapTable(legacyData: any): ComprehensiveCapTable {
    ...
    const option_pools: ComprehensiveOptionPool[] = (legacyData.options || []).map(
  ...
  static calculateMetrics(capTable: ComprehensiveCapTable) {
    const total_preferred_shares = capTable.preferred_series.reduce(
      (sum, series) => sum + series.shares_outstanding,
      0
    )

    const total_liquidation_preference = capTable.preferred_series.reduce(
      (sum, series) =>
        sum + series.shares_outstanding * series.original_issue_price * series.liquidation_multiple,
      0
    )

    const total_invested = capTable.preferred_series.reduce(
      (sum, series) => sum + series.shares_outstanding * series.original_issue_price,
      0
    )

    const total_options = capTable.option_pools.reduce(
      (sum, pool) => sum + pool.total_outstanding,
      0
    )

    const total_fully_diluted_shares =
      capTable.common_stock_shares + total_preferred_shares + total_options
    ...
    capTable.preferred_series.forEach((series) => {
      ...
      preferred_ownership_percentages[series.series_name] =
        (as_converted_shares / total_fully_diluted_shares) * 100
    })

    const common_ownership_percentage =
      (capTable.common_stock_shares / total_fully_diluted_shares) * 100

    const option_pool_percentages: { [pool_name: string]: number } = {}
    capTable.option_pools.forEach((pool) => {
      option_pool_percentages[pool.pool_name] =
        (pool.total_outstanding / total_fully_diluted_shares) * 100
    })
    ...
    return {
      ...
      liquidation_preference_coverage:
        total_invested > 0 ? total_liquidation_preference / total_invested : 1,
  ...
  static updateCalculations(capTable: ComprehensiveCapTable): ComprehensiveCapTable {
    ...
    updatedCapTable.total_authorized_shares =
      updatedCapTable.total_authorized_shares || metrics.total_fully_diluted_shares
    ...
    return {
      cap_table: {
        ...
        total_authorized_shares:
          updatedCapTable.total_authorized_shares || updatedCapTable.total_fully_diluted_shares,
  ...
  static previewWaterfallDistribution(
    ...
    const sorted_series = [...capTable.preferred_series].sort(
        ...
        sorted_series.forEach((series) => {
          if (series.is_participating && distributions[series.series_name] > 0) {
            ...
            if (series.participation_cap) {
              const lp =
                series.shares_outstanding *
                series.original_issue_price *
                series.liquidation_multiple
              const max_total =
                series.participation_cap_type === 'multiple'
