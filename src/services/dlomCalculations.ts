...
export class DLOMCalculationService {
  ...
  private calculateChaffee(inputs: DLOMInputs): number {
    ...
    const putPrice =
      K * Math.exp(-r * T) * this.normSDist(-d2) - S * Math.exp(-q * T) * this.normSDist(-d1)
  ...
  private calculateFinnerty(inputs: DLOMInputs): number {
    ...
    const d1 =
      (Math.log(S / avgStrike) + (r - q + 0.5 * sigma * sigma) * T) / (sigma * Math.sqrt(T))
    const d2 = d1 - sigma * Math.sqrt(T)

    const putPrice =
      avgStrike * Math.exp(-r * T) * this.normSDist(-d2) -
      S * Math.exp(-q * T) * this.normSDist(-d1)
    const dlom = (putPrice / S) * 100
  ...
  public getDLOMFromAssumptions(assumptions: any): {
    ...
    const inputs: DLOMInputs = {
      ...
      volatility: this.getAssumptionValue(
        assumptions,
      ...
      timeToExpiration: this.getAssumptionValue(
        assumptions,
        'volatility_assumptions',
        'time_to_liquidity',
        3
